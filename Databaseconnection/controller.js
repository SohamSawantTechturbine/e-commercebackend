...
const { mongoconnection, model, UserCart, product, productdetail } = require("./connection");
...
async function getproductdetail(req, res) {
  try {
    ...
    const existingCart = await productdetail.findOne({ userId: userId });
  ...
  try {
    if (req.user) {
      const userId = req.user.id;
        ...
        console.log(cartdata,firstName,phone)

      let existingCart = await productdetail.findOne({ userId });
      if (!existingCart) {
        existingCart = new productdetail({
          userId: userId,
        ...
        existingCart.products.push(...cartdata.map(item => ({
          productId: item.productId,
          productTitle: item.productTitle,
          productPrice: item.productPrice,
        ...
        // existingCart.firstName = firstName, // Add firstName
    ...
    res.status(500).json({ success: false, message: "Failed to update user cart data" });
  }
}
...
module.exports = { addtocart, productsdetails, logindata, registerdata, getcartdata, removeFromCart, addQuantity, removeQuantity, paymentdetails,getproductdetail ,createcheckoutsession}
